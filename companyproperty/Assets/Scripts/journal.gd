...
func _ready() -> void:
	for file in DirAccess.get_files_at("res://Assets/TextFiles/Journal/"):
		...
		journal_file_inst.get_node("HBoxContainer/Date").text = string_array[1]
		files.add_child(journal_file_inst)
		journal_file_inst.pressed.connect(Callable(game, "createWindow").bind(journal_file_inst, Vector2(256, 256), false, TEXT_FILE, string_array[2]))
